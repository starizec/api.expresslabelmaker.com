@php
    ...
    $user = $record ? User::with(['domains.licences'])->find($record->id) : null;
    $domains = $user ? $user->domains : collect();
@endphp
...
<div class="space-y-6">
    @foreach($domains as $domain)
        <div class="space-y-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ DomainsResource::getUrl('edit', ['record' => $domain->id]) }}" 
                   class="text-xs text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300 underline">
                    Uredi domenu
                </a>
            </div>

            {{-- Licenses Table --}}
            @if($domain->licences->count() > 0)
                <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($domain->licences as $licence)
                                @php
                                    $today = Carbon::now();
    ...
    @if($domains->count() === 0)
        <p class="text-sm text-gray-500 dark:text-gray-400">Korisnik nema dodijeljenih domena.</p>
    @endif
</div>
